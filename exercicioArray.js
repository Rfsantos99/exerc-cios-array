...
console.log(`A quantidade de pessoas do sexo feminino é igual a ${data.filter((fem) => fem.sexo === 'F').length}`)

//3 Imprima no console a soma do salário de todas as pessoas.
const total = data.reduce((atual, proximo)=> atual+proximo.salario,0)
console.log(`O valor total da soma dos salários é ${total}.`)

//4 Imprima no console a soma do salario de todos as pessoas do sexo Masculino
const totalMasc = data.filter((masc) => masc.sexo === 'M').reduce((atual, proximo)=> atual+proximo.salario,0)
console.log(`A soma do salário do sexo masculino é ${totalMasc}.`)
...
console.log(`o salário é maior que 7000? ${sheila}.`)
...
console.log(`Tem alguma Eva Trindade aqui? ${evinha}.`)
...
const nomes = data.map(item => console.log(item.nome))
